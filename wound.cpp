...
//--------------------------------------------------------//
// TRACTION VARING WITH STRETCH
//--------------------------------------------------------//
double evalt_rho_rho(const std::vector<Vector3d> &Phi_Lambda, double Eb, double fm, double rho_i, double time_load)
{
	//---------------------------------//
	// Function calculated by stochastic simulation. This function can be replaced by solving ODE in CPP code
	// Given Eb, f_m -> phi_b -> t_rho_rho -> SS_act
	//  Eb: Kind of the averaged strain around direction, a0
	//  f_m: contractility force [pN]
	//  lambda: corresponding stretch
	//  phi_b: probability of binding at the steady state
	//  rho_b: bond surface density
	//---------------------------------//
	double lambda = Eb + 1;
	// Read phi_b(lambda, f) from data file
	double phi_b = LineInterpolation(Phi_Lambda, lambda, 0);
	if(phi_b==-10000){
		std::cout<<"lambda = "<<lambda<<"\n";
 		std::cout<<"The value of lambda is out of range!\n";
		return -1; // error flag, may need to change
	}
	// Check read phi_b
	//std::cout<<phi_b<<"\n";

	double rho_b = phi_b*rho_i; //from rho_i=500 [mu m]^-2
	// T_A = f_m * rho_b
	double t_rho_rho = 1.0e-6 * fm * rho_b * time_load; //[pN/(mu m)^2 to MPa]
	return t_rho_rho;
}

double evalDt_rho_rhoDEb(const std::vector<Vector3d> &Phi_Lambda, double Eb, double fm, double rho_i, double time_load)
{
	double lambda = Eb + 1;
	// Read Dphi_b(lambda, f) from data file
	double Dphi_b = LineInterpolation(Phi_Lambda, lambda, 1);
	if(Dphi_b==-10000){
		std::cout<<"lambda = "<<lambda<<"\n";
		std::cout<<"The value of lambda is out of range!\n";
		return -1; // error flag, may need to change
	...
	double Dt_rho_rhoDEb = 1.0e-6 * fm * rho_i * Dphi_b * time_load;
	return Dt_rho_rhoDEb;
}


double readPhib(std::string fiber_ref, std::vector<Vector3d> &Phi_Lambda, double fm)
{
  //This function read data from "filename_prefix+fm.txt" text file
  //  fm is the contractility force to be used;
  //  the data file must be in the format of "Lambda  Phi_b  dPhi_bdLambda";

  // Choose phi_b data file with specific f_m
  std::stringstream ss;
  ss << fm;
  // Currently available data sets:
  //    -- ColSvensson1f
  //    -- ColSvensson2f
  //    -- FibrinWLiuUncrossForward
  //    -- FibronectinDeraviCirc
  //    -- FibronectinDeraviSqua
  //    -- FibronectinDeraviTri
  std::string filename = "PhibCurve/PhibLamb_"+fiber_ref+"f" + ss.str()+".txt";
  // READ
  Phi_Lambda.clear();
    ...
    while ( getline(myfile,line) )
      {
        ...
        Phi_Lambda.push_back(Vector3d(std::stod(strs[0]),std::stod(strs[1]),std::stod(strs[2])));
      }
  }
  myfile.close();

  //Check inputs
  //std::cout<<"Lambda, Phi inputs:\n";
  //for(int ilambda=0;ilambda<Phi_Lambda.size();ilambda++){
  //  std::cout<<Phi_Lambda[ilambda](0)<<","<<Phi_Lambda[ilambda](1)<<"\n";
  //}
  return 0;
}
...
double LineInterpolation(const std::vector<Vector3d> Phi_Lambda, double lambda, int derivOrder)
{
  ...
  double x0, x1, y0, y1, xi, yi; // for interpolation

  //Find lambda's position
  for(int i=0; i<Phi_Lambda.size()-1; i++){
    if(lambda>=Phi_Lambda[i](0) && lambda<Phi_Lambda[i+1](0)){
      x0 = Phi_Lambda[i](0);
      x1 = Phi_Lambda[i+1](0);
      xi = lambda;
      y0 = Phi_Lambda[i](derivOrder+1);
      y1 = Phi_Lambda[i+1](derivOrder+1);
      yi = (y1-y0)*(xi-x0)/(x1-x0) + y0;
      return yi;
    }
  }
  //  derivOrder is the derivative order of phi_b wrt lambda, i.e.
  //    0 -- Phi_b
  //    1 -- dPhi_bdLambda
...
VectorXd &Re_c,MatrixXd &Ke_c_x,MatrixXd &Ke_c_rho,MatrixXd &Ke_c_c)
{
	...
	//double f_m = global_parameters[21]; // contractile force
	//double rho_i = global_parameters[22]; // density of integrins
	...
	std::vector<Vector3d> Phi_Lambda; Phi_Lambda.clear(); //Initialization
	std::vector<Vector3d> Phi_Lambda_health; Phi_Lambda_health.clear();
	std::vector<Vector3d> Phi_Lambda_wound; Phi_Lambda_wound.clear();
	readPhib(fiber_health, Phi_Lambda_health, global_parameters[21]);
	//readPhib(fiber_wound, Phi_Lambda_wound, f_m);
	//---------------------------------//
...
//		std::cout<<"gradient of c: "<<Grad_c<<"\n";
		...
		//std::cout<<"CC = "<<CC(0,0)<<","<<CC(0,1)<<","<<CC(1,0)<<","<<CC(1,1)<<"\n";
		//std::cout<<"rho="<<rho<<", c="<<c<<"\n";
		...
		double rho_i = ip_rhoi[ip];
		double f_m = ip_fm[ip];
		//Set up fiber types for healthy and wounded parts
		if(ip_fiber[ip] == 0){
			Phi_Lambda = Phi_Lambda_health;
			k0 = global_parameters[0];
			kf = global_parameters[1];
			k2 = global_parameters[2];
		}else if(ip_fiber[ip] == 1){
			readPhib(fiber_wound, Phi_Lambda_wound, f_m);
			Phi_Lambda = Phi_Lambda_wound;
			k0 = global_parameters[23];
			kf = global_parameters[24];
			k2 = global_parameters[25];
		} else {
			std::cout<<"Cannot read fiber data!";
			return;
		}
		...
		// need to be reconsidered if remodeling exists
		CCinv = CC.inverse();
		Matrix2d Rot90;Rot90<<0.,-1.,1.,0.;
		...
		Matrix2d SSe_pas;SSe_pas<<0.,0.,0.,0.;
		Matrix2d SS_pas;SS_pas<<0.,0.,0.,0.;
		Matrix2d SS_act;SS_act<<0.,0.,0.,0.;
		std::vector<Vector2d> a0_i;a0_i.clear();

		// Define number of orientations
		int n_orientation = 11; //this n is the number of orientation in [-pi/2,pi/2]
		std::vector<double> P_theta_i(n_orientation); // Store PDF(theta_i
		std::vector<double> theta_disperion(n_orientation);
		std::vector<double> I4e(n_orientation);
		std::vector<double> trA(n_orientation);
		std::vector<double> traction_act(n_orientation);

		//double Psum=0;
		for(int itheta=0;itheta<n_orientation;itheta++){
			theta_disperion[itheta] = (2.0*itheta+1.0-n_orientation)/(2.0*n_orientation)*PIE;
			P_theta_i[itheta] = PIE/n_orientation*exp(kappa*cos(2.0*theta_disperion[itheta]))/(PIE*cyl_bessel_i(0,kappa)); // Probability of von-Mises distribution
			//Psum += P_theta_i[itheta];
			Matrix2d RotTheta;RotTheta<<cos(theta_disperion[itheta]),-sin(theta_disperion[itheta]),sin(theta_disperion[itheta]),cos(theta_disperion[itheta]);
//			std::cout<<RotTheta<<"tag1\n";
			a0_i.push_back(RotTheta*a0);
			// re-compute basis a0, s0
			Vector2d s0_i = Rot90*a0_i[itheta];
			// fiber tensor in the reference
			Matrix2d a0a0_i = a0_i[itheta]*a0_i[itheta].transpose();
			Matrix2d s0s0_i = s0_i*s0_i.transpose();
			Matrix2d A0 = a0a0_i;
			Vector2d a_i = FF*a0_i[itheta];
			Matrix2d A = a_i*a_i.transpose();
			trA[itheta]=A(0,0) + A(1,1);
			Matrix2d hat_A = A/trA[itheta];
			// recompute split
			//Matrix2d FFg = lamdaP_a*(a0a0_i) + lamdaP_s*(s0s0_i);
			//Matrix2d FFginv = (1./lamdaP_a)*(a0a0_i) + (1./lamdaP_s)*(s0s0_i);
			//Matrix2d FFe = FF*FFginv;
			...
			I4e[itheta]=a0_i[itheta].dot(CCe*a0_i[itheta]);
			// calculate the normal stretch
			//double thetaE = sqrt(CCe.determinant());
			//double theta = thetaE*thetaP;
			//std::cout<<"split of the determinants. theta = thetaE*thetaB = "<<theta<<" = "<<thetaE<<"*"<<thetaP<<"\n";
			//double lamda_N = 1./thetaE;
			double I4tot = a0_i[itheta].dot(CC*a0_i[itheta]);
			// Second Piola Kirchhoff stress tensor
			// passive elastic
			// strain energy without quadractic exponential index
			double Psif = kf/(2.*k2)*(exp(k2*(I4e[itheta] -1)) - k2*(I4e[itheta]-1) - 1);
			//double Psif1 = kf/2*kappa* (exp(k2*(kappa*I1e + (1-2*kappa)*I4e[itheta] -1)) - 1);
			double Psif4 = kf/2*(exp(k2*(I4e[itheta]-1)) - 1);
			SSe_pas += phif*(P_theta_i[itheta]*Psif4*a0a0_i);
			// pull back to the reference
			// magnitude from systems bio
			//double traction_act = (t_rho + t_rho_c*c/(K_t_c + c))*rho;
			// Kind of an average strain (referred to HGO model paper)
			double Eb = sqrt(I4e[itheta]) - 1;
			if(Eb < 0){
				//For non-stretched case
				Eb = 0;
			}
			//std::cout<<ip_fiber[ip]<<" "<<f_m<<" "<<rho_i<<"\n";
			traction_act[itheta]=evalt_rho_rho(Phi_Lambda, Eb, f_m, rho_i, time_load);
//			double traction_act = (evalt_rho_rho(Phi_Lambda, Eb, f_m, rho_i, time_load) + t_rho_c*c/(K_t_c + c))*rho;
			SS_act += P_theta_i[itheta]*(thetaP*traction_act[itheta]*phif/trA[itheta])*A0;
		}

		SSe_pas += k0*Identity;
		SS_pas = thetaP*FFginv*SSe_pas*FFginv;
		//std::cout<<"Sact"<<SS_act<<", \nSpas"<<SS_pas<<"\n";
		// total stress, don't forget the pressure
		double pressure = -1.0*k0*lamda_N*lamda_N;
		Matrix2d SS_pres = pressure*thetaP*CCinv;
		//std::cout<<"stresses.\nSSpas\n"<<SS_pas<<"\nSS_act\n"<<SS_act<<"\nSS_pres"<<SS_pres<<"\n";

		Matrix2d SS = SS_pas + SS_act + SS_pres; // SS here is dpsidCC since we use S:deltaC for Re_x
		Vector3d SS_voigt = Vector3d(SS(0,0),SS(1,1),SS(0,1));
		//std::cout<<"stress in voigt\n"<<SS_voigt<<"\n";
		Matrix2d sigma_current = 2*FF*SS*FF.transpose();
		ip_sigma[ip](0) = sigma_current(0,0); // sigma_total_11
		ip_sigma[ip](1) = sigma_current(1,1); // sigma_total_22
		ip_sigma[ip](2) = sigma_current(0,1); // sigma_total_12
		//ip_sigma[ip](2) = f_m;

		sigma_current = 2*FF*SS_act*FF.transpose();
		ip_sigma_act[ip](0) = sigma_current(0,0); // sigma_act_11
		ip_sigma_act[ip](1) = sigma_current(1,1); // sigma_act_22
		ip_sigma_act[ip](2) = sigma_current(0,1); // sigma_act_12

		sigma_current = 2*FF*(SS_pas+SS_pres)*FF.transpose();
		ip_sigma_pas[ip](0) = sigma_current(0,0); // sigma_pas_11
		ip_sigma_pas[ip](1) = sigma_current(1,1); // sigma_pas_22
		ip_sigma_pas[ip](2) = sigma_current(0,1); // sigma_pas_12
		...
		for(int nodei=0;nodei<4;nodei++){
			for(int coordi=0;coordi<2;coordi++){
				...
				Re_x(nodei*2+coordi) += Jac*wip*SS_voigt.dot(deltaCC_voigt);
			}
		}
		// 
		...
		//
		// phif
		evalFluxesSources(global_parameters,ip_fiber[ip],phif_plus,a0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus, time_load, Phi_Lambda);
		evalFluxesSources(global_parameters,ip_fiber[ip],phif_minus,a0,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus, time_load, Phi_Lambda);
		...
		//
		// a0x
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0_plus_x,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus, time_load, Phi_Lambda);
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0_minus_x,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus, time_load, Phi_Lambda);
		...
		//
		// a0y
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0_plus_y,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus, time_load, Phi_Lambda);
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0_minus_y,kappa,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus, time_load, Phi_Lambda);
		...
		//
		// kappa
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0,kappa_plus,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus, time_load, Phi_Lambda);
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0,kappa_minus,lamdaP,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus, time_load, Phi_Lambda);
		...
		//
		// lamdaP_a
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0,kappa,lamdaP_plus_a,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus, time_load, Phi_Lambda);
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0,kappa,lamdaP_minus_a,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus, time_load, Phi_Lambda);
		...
		//
		// lamdaP_s
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0,kappa,lamdaP_plus_s,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_plus,Q_rho_plus,S_rho_plus,Q_c_plus,S_c_plus, time_load, Phi_Lambda);
		evalFluxesSources(global_parameters,ip_fiber[ip],phif,a0,kappa,lamdaP_minus_s,FF,rho,c,Grad_rho,Grad_c,f_m,rho_i, SS_minus,Q_rho_minus,S_rho_minus,Q_c_minus,S_c_minus, time_load, Phi_Lambda);
		...
		//---------------------------------//
		// MECHANICS TANGENT
		// use modified HGO strain energy
		//double Psif11 = kf/2*k2*kappa*kappa * exp(k2 * (kappa*I1e+(1-2*kappa)*I4e-1));
		//double Psif14 = kf/2*k2*kappa*(1-2*kappa) * exp(k2 * (kappa*I1e+(1-2*kappa)*I4e-1));
		//double Psif41 = kf/2*k2*kappa*(1-2*kappa) * exp(k2 * (kappa*I1e+(1-2*kappa)*I4e-1));
		...
		for(ii=0;ii<2;ii++){
			for(jj=0;jj<2;jj++){
				for(kk=0;kk<2;kk++){
					for(ll=0;ll<2;ll++){
						dSSpasdCC_explicit[ii*8+jj*4+kk*2+ll] = 0;
						for(int itheta=0;itheta<n_orientation;itheta++){
							Matrix2d a0a0_i = a0_i[itheta]*a0_i[itheta].transpose();
							double Psif44 = kf/2*k2*exp(k2*(I4e[itheta]-1));
							dSSpasdCC_explicit[ii*8+jj*4+kk*2+ll] += 
												//	phif*(Psif11*Identity(kk,ll)*Identity(ii,jj) + Psif14*Identity(ii,jj)*a0a0(kk,ll)
												//  +Psif41*a0a0(ii,jj)*Identity(kk,ll) + Psif44*a0a0(ii,jj)*a0a0(kk,ll));
													phif*(P_theta_i[itheta]*Psif44*a0a0_i(ii,jj)*a0a0_i(kk,ll));
						}
					}	
				}
			}
		...
		for(int II=0;II<3;II++){
			for(int JJ=0;JJ<3;JJ++){
				...
				// codde similar to homework 5, since Fg = identity so no need to worry about that 
//				DDpas(II,JJ) = Psif11*Identity(ii,jj)*Identity(kk,ll) + Psif14*Identity(ii,jj)*a0a0(kk,ll)+Psif14*

				DDact(II,JJ) = 0;
				for(int itheta=0;itheta<n_orientation;itheta++){
					Matrix2d a0a0_i = a0_i[itheta]*a0_i[itheta].transpose();
					Matrix2d dtrAdCC = a0a0_i;
					Matrix2d A0 = a0a0_i;
					double Eb = sqrt(I4e[itheta]) - 1;
					if(Eb < 0){
						//For non-stretched case
					...
					DDact(II,JJ) += P_theta_i[itheta]*phif*( -1.0*traction_act[itheta]/(trA[itheta]*trA[itheta])*dtrAdCC(kk,ll)*A0(ii,jj)
									+ evalDt_rho_rhoDEb(Phi_Lambda, Eb, f_m, rho_i, time_load)*0.5/(Eb+1.0) / (trA[itheta])*A0(ii,jj) * a0a0_i(kk,ll));
					//std::cout<<"DDact: "<<DDact(II,JJ)<<"\n";
				}
				// structural
				...
				//evalSS(global_parameters,phif,a0,kappa,lamdaP_a,lamdaP_s,CC_p,rho,c,SSpas_p,SSact_p,SSpres_p,time_load,Phi_Lambda);
				//evalSS(global_parameters,phif,a0,kappa,lamdaP_a,lamdaP_s,CC_m,rho,c,SSpas_m,SSact_m,SSpres_m,time_load,Phi_Lambda);
		...
		dtractiondrho = 0;
		Matrix2d dSSdrho_explicit = (thetaP*dtractiondrho*phif/trA[(n_orientation-1)/2])*(kappa*Identity+(1-2*kappa)*a0a0);
		...
		dtractiondc = 0;
		Matrix2d dSSdc_explicit = (thetaP*dtractiondc*phif/trA[(n_orientation-1)/2])*(kappa*Identity+(1-2*kappa)*a0a0);
			...
			for(int coordi=0;coordi<2;coordi++){
				...
				for(int nodej=0;nodej<4;nodej++){
					for(int coordj=0;coordj<2;coordj++){
						...
						//
						Ke_x_x(nodei*2+coordi,nodej*2+coordj) += Jac*deltaCC_voigt.dot(DDtot*linCC_voigt)*wip;
						...
						//
						Ke_x_x(nodei*2+coordi,nodej*2+coordj) += Jac*SS_voigt.dot(lindeltaCC_voigt)*wip;
		...
		// wrt Mechanics
		Matrix2d dHedCC_explicit = -1./pow((1.+exp(-gamma_c_thetaE*(thetaE + theta_phy))),2)*(exp(-gamma_c_thetaE*(thetaE + theta_phy)))*(-gamma_c_thetaE)*(1./thetaP)*(dthetadCC);
		...
		for(int nodei=0;nodei<4;nodei++){
			for(int nodej=0;nodej<4;nodej++){
				for(int coordj=0;coordj<2;coordj++){
					...
					Ke_rho_x(nodei,nodej*2+coordj)+= -R[nodei]*dS_rhodCC_voigt.dot(linCC_voigt)*Jac;
					
					//-----------//
					// Ke_c_X
					//-----------//

					Ke_c_x(nodei,nodej*2+coordj)+= -R[nodei]*dS_cdCC_voigt.dot(linCC_voigt)*Jac;
...
void evalFluxesSources(const std::vector<double> &global_parameters,int ip_fiber_type,double phif,Vector2d a0,double kappa,Vector2d lamdaP,
...
double time_load, const std::vector<Vector3d> &Phi_Lambda)
{
	...
	double p_rho =global_parameters[9]; // production of fibroblasts naturally
	double p_rho_c = global_parameters[10]; // production enhanced by the chem
	double p_rho_theta = global_parameters[11]; // enhanced production by deformation
	double K_rho_c= global_parameters[12]; // saturation of cell production by chemical
	double K_rho_rho = global_parameters[13]; // saturation of cell by cell
	double d_rho = global_parameters[14] ;// decay of cells
	double theta_phy = global_parameters[15]; // physiological state of area stretch
	...
	for(int itheta=0;itheta<n_orientation;itheta++){
		theta_disperion[itheta] = (2.0*itheta+1.0-n_orientation)/(2.0*n_orientation)*PIE;
		P_theta_i[itheta] = PIE/n_orientation*exp(kappa*cos(2.0*theta_disperion[itheta]))/(PIE*cyl_bessel_i(0,kappa)); // PDF of von-Mises distribution
		//Psum += P_theta_i[itheta];
...
VectorXd &dThetadCC, VectorXd &dThetadrho, VectorXd &dThetadc)
{
	...
	//
	// solution parameters
	double tol_local = 1e-4; // local tolerance
	int max_local_iter = 100; // max local iter
	//
	// other local stuff
	Matrix2d Identity;Identity<<1.,0.,0.,1.;
	...
	while(residuum>tol_local && iter<max_local_iter){
		...
		iter += 1;
		if(iter==max_local_iter and normRR>1e-4){
	...
	// phi
	double dphif_dot_plusdc = (rho/(K_phi_rho+phif))*((p_phi_c)/(K_phi_c+c) - (p_phi_c*c)/((K_phi_c+c)*(K_phi_c+c)))-d_phi_rho_c*rho*phif;
	double dRphidc = dphif_dot_plusdc;
...
void evalPsif(const std::vector<double> &global_parameters,double kappa, double I1e,double I4e,double &Psif,double &Psif1,double &Psif4)
{
	...
	double k2 = global_parameters[2]; // nonlinear exponential
	// passive elastic
	//Psif = (kf/(2.*k2))*exp( k2*pow((kappa*I1e + (1-2*kappa)*I4e -1),2));
	//Psif1 = 2*k2*kappa*(kappa*I1e + (1-2*kappa)*I4e -1)*Psif;
	//Psif4 = 2*k2*(1-2*kappa)*(kappa*I1e + (1-2*kappa)*I4e -1)*Psif;
	// strain energy without quadractic exponential index
	Psif = kf/(2.*k2)*(exp(k2*(kappa*I1e + (1-2*kappa)*I4e -1)) - k2*(kappa*I1e + (1-2*kappa)*I4e - 1));
	Psif1 = kf/2*kappa* (exp(k2*(kappa*I1e + (1-2*kappa)*I4e -1)) - 1);
	Psif4 = kf/2*(1-2*kappa)* (exp( k2*(kappa*I1e + (1-2*kappa)*I4e-1)) - 1);
}
	...
	double time_load, std::vector<Vector3d> &Phi_Lambda)
{
	...
	//double traction_act = (t_rho + t_rho_c*c/(K_t_c + c))*rho;
	// Kind of an average strain (referred to HGO model paper)
        double Eb = kappa*I1e + (1-2*kappa)*I4e - 1;
        if(Eb < 0){
		//For non-stretched case
		Eb = 0;
	}
	double traction_act = evalt_rho_rho(Phi_Lambda, Eb, f_m, rho_i, time_load);
//	double traction_act = (evalt_rho_rho(Phi_Lambda, Eb, f_m, rho_i, time_load) + t_rho_c*c/(K_t_c + c))*rho;
...
VectorXd &Re_x,VectorXd &Re_rho,VectorXd &Re_c)
{
	...
	Matrix2d Rot90;Rot90<<0.,-1.,1.,0.;
	Ebasis.push_back(Vector2d(1.,0.)); Ebasis.push_back(Vector2d(0.,1.));
	std::vector<Vector3d> Phi_Lambda; Phi_Lambda.clear();
	std::vector<Vector3d> Phi_Lambda_health; Phi_Lambda_health.clear();
	std::vector<Vector3d> Phi_Lambda_wound; Phi_Lambda_wound.clear();
	readPhib(fiber_health, Phi_Lambda_health, f_m);
	readPhib(fiber_wound, Phi_Lambda_wound, f_m);
	//---------------------------------//
	...
	for(int ip=0;ip<IP.size();ip++)
	{
		...
		// Second Piola Kirchhoff stress tensor
		// passive elastic
		//double Psif = (kf/(2.*k2))*exp( k2*pow((kappa*I1e + (1-2*kappa)*I4e -1),2));
		//double Psif1 = 2*k2*kappa*(kappa*I1e + (1-2*kappa)*I4e -1)*Psif;
		//double Psif4 = 2*k2*(1-2*kappa)*(kappa*I1e + (1-2*kappa)*I4e -1)*Psif;
		// strain energy without quadractic exponential index
		double Psif = kf/(2.*k2)*(exp(k2*(kappa*I1e + (1-2*kappa)*I4e -1)) - k2*(kappa*I1e + (1-2*kappa)*I4e - 1));
		double Psif1 = kf/2*kappa* (exp(k2*(kappa*I1e + (1-2*kappa)*I4e -1)) - 1);
		double Psif4 = kf/2*(1-2*kappa)* (exp( k2*(kappa*I1e + (1-2*kappa)*I4e-1)) - 1);
		Matrix2d SSe_pas = k0*Identity + phif*(Psif1*Identity + Psif4*a0a0);
		// pull back to the reference
		Matrix2d SS_pas = thetaP*FFginv*SSe_pas*FFginv;
		// magnitude from systems bioß
		//double traction_act = (t_rho + t_rho_c*c/(K_t_c + c))*rho;
		// Kind of an average strain (referred to HGO model paper)
		double Eb = kappa*I1e + (1-2*kappa)*I4e - 1;
		if(Eb < 0){
